...
import { createCoinbaseCharge } from '../services/coinbase'; // Added for Coinbase
...
export const resolvers = {
  ...
  Mutation: {
    ...
    createSubscription: async (_: any, { input }: { input: any }, { user }: { user: any }) => {
      if (!user) throw new AuthenticationError('Must be authenticated');
      const { tier, newCoinbaseCharge } = input;
      ...
      let subscription;
      if (newCoinbaseCharge) {
        const userSubscription = await subscriptionService.getUserSubscription(user.id);
        const canUseCoinbase = await subscriptionService.checkCoinbaseUsage(user.id, userSubscription.tier);
        if (!canUseCoinbase) {
          throw new ForbiddenError('Coinbase payment not allowed for this subscription tier');
        }
        const charge = await createCoinbaseCharge({
          name: `OmniAuthor ${tier} Subscription`,
          description: `Subscription for ${user.email}`,
          amount: SUBSCRIPTION_PLANS[tier].price,
          currency: 'USD',
          userId: user.id,
        });
        subscription = await subscriptionService.createSubscription(user.id, tier, {
          chargeId: charge.id,
    ...
    createCoinbaseCharge: async (_: any, { input }: { input: any }, { user }: { user: any }) => {
      if (!user) throw new AuthenticationError('Must be authenticated');
      const { name, description, amount, currency } = input;
      const userSubscription = await subscriptionService.getUserSubscription(user.id);
      const canUseCoinbase = await subscriptionService.checkCoinbaseUsage(user.id, userSubscription.tier);
      if (!canUseCoinbase) {
        throw new ForbiddenError('Coinbase payment not allowed for this subscription tier');
      ...
      logger.info(`Coinbase charge created: ${charge.id} for user: ${user.id}`);
      return charge;
    },
